...
A and B can publicly agree on a secret key while communicating over a public/insecure network!

### Flavour
Given <b>A</b>,<b>B</b> and eavesdropper <b>C</b> and  assuming color seperation is expensive: 
...
A and B publicly agree on a prime number <b>P</b> (say 23). A and B then agree on a generator <b>G</b> (say 5) belonging to {1, ... , P-1}. P and G are chosen so that shared secret could take any value between 1 to p-1.
...
Even though C is aware of P, G, A1 and B1; C is unable to generate the new secret key as it is computationally intractable to find secret keys from A1 and B1.
...
What are groups and how do we find generators?

### Group
A group is a set with operation under : <i> closure , existence of inverse element , associativity, existence of identity element </i>. For instnce, (Z,+) is integer group under addition.
...
Example
Consider a dot operation defined as <b>a . b = (a*b) mod n</b> over a simple group G : {1,2,3,4} for n = 5
Also consider the notation that a^i = ((a . a ). a) ... (i times).
Now, a number i from G will be called a generator if it performs the operation on itself to generate the whole group.

- 1 : 1^i = 1.1.1.1.1 = {1} for any i
- 2 : 2^i = {1,2,3,4} for i=4,1,3,2 respectively 
- 3 : 3^i = {1,2,3,4} for i=4,3,1,2 respectively
- 4 : 4^i = {1,4} for i=2,1 and so on..

Thus we call 2 and 3 to be <i>generators of group (G,dot) </i>. These generators make all values of the group to be equally likely to be the encryption key. The larger the p, the safer the encryption key.

The above is an example of a [<i>multiplicative group of integers modulo <b>n</b></i>](https://en.wikipedia.org/wiki/Multiplicative_group_of_integers_modulo_n#Cyclic_case). It is defined as a group of integers coprime to <b>n</b> from the set {0,1,...n-1} under <i>multiplication modulo n</i>.
